  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      querySnapshot.forEach(async (doc) => {
        setSubscription({
          unit_amount: doc.data().items[0]?.price?.unit_amount,
    ...
    onSnapshot(docRef, async (snapshot) => {
      ...
      if (sessionId) {
        const stripe = await loadStripe(
          "pk_test_51Lo57ASCFsf4tL5XQ2ycs2UW1lCnkmDRt0KC9CGP5NF87HnSGLmPP7hf0BA7Fwi2W8UKAptgicgmyiWs9mEsRpIP00RJDzvCNQ"
        );
  ...
  return (
    <div className="plans">
      ...
      {Object.entries(products).map(([productId, productData]) => {
        var plan_name =null;
        if(subscription?.unit_amount===14900){
          plan_name='mobile'
        }
        else if(subscription?.unit_amount===19000){
          plan_name='basic'
        }
        else if(subscription?.unit_amount===49900){
          plan_name='standard'
        }
        else if(subscription?.unit_amount===64900){
          plan_name='premium'
        }
